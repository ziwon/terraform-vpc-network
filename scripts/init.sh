...
CURRENT_DIR="$( cd "$( dirname "${BASH_SOURCE[1]}" )" >/dev/null 2>&1 && pwd )"

CONFIG_HCL=${CURRENT_DIR}/config/backend_config.hcl
CONFIG_TPL=${CURRENT_DIR}/config/backend_config.tpl

AWS_REGION=${AWS_REGION:-ap-northeast-2}

# we have one prod environment only for global infra
ENV=${1:-prod}
